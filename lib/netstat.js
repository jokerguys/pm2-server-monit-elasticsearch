...
function refreshIp(interval) {
  exec('ip -s link', function(err, out) {
    ...
    if (stats.length > 0) {
      metrics.total.input.value = totalIn.toFixed(2) + 'MB/s';
      metrics.total.output.value = totalOut.toFixed(2) + 'MB/s';
      totalIn = 0;
      totalOut = 0;
    }
    oldStats = stats;
...
function refreshIfconfig(interval) {
  ...
  exec('/sbin/ifconfig', function(err, out) {
    if (err) {
        // console.log(err);
      metrics.total.input.value = '0';
      metrics.total.output.value = '0';
      return;
    }
    var totalRx = 0;
    var totalTx = 0;
    ...
    blocks.forEach(function (block) {
      ...
      if (block.device != 'lo' && block.device != 'lo0' && block.other && block.other.rxBytes > 0 && block.other.txBytes > 0) {
        ...
        if (old) {
          metrics[block.device].input.value = (((block.other.rxBytes - old.other.rxBytes) / interval) / 1000000).toFixed(2) + ' MB/s';
          metrics[block.device].output.value = (((block.other.txBytes - old.other.txBytes) / interval) / 1000000).toFixed(2) + ' MB/s';
    ...
    metrics.total.input.value = ((totalRx / 1000000) / interval).toFixed(2) + ' MB/s';
    metrics.total.output.value = ((totalTx / 1000000) / interval).toFixed(2) + ' MB/s';
...
function init(conf) {
  ...
  setInterval(function() {
    refreshIp(conf.small_interval)
  }, conf.small_interval * 1000);
}
